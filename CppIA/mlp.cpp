...
MLP::MLP(Model3 *W, Model *d, Model2 *X, Model2 *deltas) {
    this->W = W;
    this->d = d;
    this->X = X;
    this->deltas = deltas;
...
void MLP::forward_pass(float *sample_inputs, bool is_classification) {
    int L = this->d->size - 1;
...
void MLP::train_stochastic_gradient_backpropagation(float *flattened_dataset_inputs,
    ...
    for (int it = 0; it < iterations_count; it++) {
        ...
        float *sample_expected_output = cut_float_array(flattened_dataset_expected_outputs, k * output_dim,
                                                        (k + 1) * output_dim);
        ...
        for (int l = L; l >= 1; l--) {
            for (int i = 1; i < this->d->values[l - 1] + 1; i++) {
                ...
                this->deltas->values[l - 1][i] =
                        (1 - this->X->values[l - 1][i] * this->X->values[l - 1][i]) * sum_result;
            }
        }
        for (int l = 1; l < L + 1; l++) {
            for (int i = 0; i < this->d->values[l - 1] + 1; i++) {
