...
class linearModel():
    def __init__(self):
        ...
        # ----------------------------------------------------------------------------------
        self.mylib.predict_linear_model_regression_unefficient_but_more_readable.argtype = [POINTER(Model),
    ...
    def train_classification_rosenblatt_rule_linear_model(self, model, flattened_dataset_inputs,
        ...
        flattened_dataset_expected_outputs_cast = cast(
            (c_float * len(flattened_dataset_expected_outputs))(*flattened_dataset_expected_outputs),
            self.c_float_p)

        self.mylib.train_classification_rosenblatt_rule_linear_model(model, flattened_dataset_inputs_cast,
    ...
    def predict_linear_model_regression_unefficient_but_more_readable(self, model, sample_inputs):
        ...
        return self.mylib.train_classification_rosenblatt_rule_linear_model(model, sample_inputs_cast,
                                                                            sample_inputs_size)

    def train_regression_pseudo_inverse_linear_model(self, model, flattened_dataset_inputs,
                                                     flattened_dataset_expected_output):
        flattened_dataset_inputs_size = len(flattened_dataset_inputs)
        ...
        flattened_dataset_inputs_cast = cast((c_float * len(flattened_dataset_inputs))(*flattened_dataset_inputs),
                                             self.c_float_p)
        flattened_dataset_expected_outputs_cast = cast(
            (c_float * len(flattened_dataset_expected_output))(*flattened_dataset_expected_output),
            self.c_float_p)

        self.mylib.train_regression_pseudo_inverse_linear_model(model, flattened_dataset_inputs_size,
    ...
    def load_linear_model(self,filename):
        ...
        return self.mylib.load_linear_model(filename_b)